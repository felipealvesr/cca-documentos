...
export async function bundleLocal(entry, outfile) {
  ...
  await build({
    stdin: { contents: await readFile(source, 'utf8'), sourcefile: path.basename(source), loader: 'ts', resolveDir: path.dirname(source) },
    bundle: true, platform: 'node', format: 'cjs', outfile: path.resolve(root, outfile), packages: 'external', plugins: [resolveLocal]
  });
}
